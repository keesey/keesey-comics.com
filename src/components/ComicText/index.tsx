...
const ComicText: FC<Props> = ({ children, emphatic, lang }) => {
    ...
    if (lang === "ko") {
        return (
            <span lang={lang} className={[styles.regular, emphatic ? styles.emphatic : null].filter(Boolean).join(" ")}>
                {children}
            </span>
        )
    }
    return (
        <>
            <span lang={lang} className={regularClassName}>
                {children}
            </span>
            <span lang={lang} className={comixifiedClassName}>
                {comixify(children)}
            </span>
        </>
    )
