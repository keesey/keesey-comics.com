...
const getVolume = (c: Container) => c.dimensions.reduce<number>((prev, measurement) => prev * measurement, 1)
...
const getShippingOptionIdForProductType = (order: Order, type: ProductType) => {
    return order.shippingOptionIds.find(id => type.shippingOptions?.some(option => option.id === id))
}
        ...
        order.items.every(({ productId, quantity }) =>
            container.quantities.some(
                ({ maximum, productType, shippingOption }) =>
                    ...
                    (!shippingOption ||
                        shippingOption.id === getShippingOptionIdForProductType(order, PRODUCTS_MAP[productId].type)) && // Includes the shipping option (if any)
