...
import { PRODUCTS, PRODUCTS_MAP } from "../../constants/PRODUCTS"
import {
  SHIPPING_OPTION_LISTS,
  SHIPPING_OPTIONS,
} from "../../constants/SHIPPING_OPTIONS"
...
const setQuantity = (
  ...
  quantity = Math.max(
    1,
    Math.min(PRODUCTS_MAP[productId].type.maximum, Math.round(quantity)),
  )
  ...
  switch (action.type) {
    ...
    case "INITIALIZE": {
      const items = (action.payload?.items ?? [])
        .filter(
          item =>
            PRODUCTS_MAP[item.productId] && isValidQuantity(item.quantity),
        )
        .map(item => ({
          ...item,
          quantity: Math.min(
            PRODUCTS_MAP[item.productId].type.maximum,
            item.quantity,
          ),
        }))
      return {
