...
import type { ServiceOption } from "../models/ServiceOption"
...
const getServiceOptionFromXML = (xml: string): ServiceOption => {
  const responseDoc = new XmlDocument(xml)
  if (
    responseDoc.name !== "RateV4Response" &&
    responseDoc.name !== "IntlRateV2Response"
  ) {
    throw new Error(
      `Unrecognized response type from USPS: "${responseDoc.name}"`,
    )
  }
  const pkg = responseDoc
    .childrenNamed("Package")
    .filter(element => element.attr.ID === "1")[0]
  if (!pkg) {
  ...
  if (error) {
    throw new Error(
      error.childNamed("Description")?.val ?? "Unknown error in USPS response.",
    )
  }
  ...
  const postage = pkg
    ...
    .filter(
      ({ rate, serviceName }) => isFinite(rate) && rate >= 0 && serviceName,
    )
